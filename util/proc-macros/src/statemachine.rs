...
impl Parse for StructFn {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        let attrs = input.call(Attribute::parse_outer)?;
        let pub_kw: Option<_> = input.parse()?;
...
impl ToTokens for StructDef {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        tokens.extend(quote! {
            #(#attrs)*
            struct #ident { #fields }
            static #ident: ::util::spin::Mutex<#ident> = ::util::spin::Mutex::new(#ident{
                #initializers
            });
        });
    }
...
impl ToTokens for StructFn {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        tokens.extend(quote! {
            #(#attrs)*
            #pub_kw fn #ident (& #mut_token self #sep #args) #ret_token #output #block
        })
    }
}
